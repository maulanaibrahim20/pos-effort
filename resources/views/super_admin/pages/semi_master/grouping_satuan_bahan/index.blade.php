@extends('index')
@section('title', 'Grouping Satuan Bahan')
@section('content')
    ...
    <div class="row">
        <div class="col-lg-4">
            @include('super_admin.pages.semi_master.grouping_satuan_bahan.create')
        </div>
        <div class="col-lg-8">
            <div class="card">
                @if (session('success'))
                            ...
                            <tbody>
                                @foreach ($groupingBahan as $data)
                                    <tr>
                                        ...
                                        <td>{{ $data->getSatuanBahan->satuanBahan }}</td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <form id="deleteForm{{ $data->id }}"
                                                action="{{ url('/super_admin/semi_master/grouping_satuan_bahan/' . $data->id) }}"
        ...
        <div class="modal fade" id="exampleModal3{{ $item->id }}" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ url('super_admin/semi_master/grouping_satuan_bahan/' . $item->id) }}" method="POST"
                                ...
                                <select name="satuanBahan" class="form-control select2 form-select" id="satuanBahan">
                                    ...
                                    @foreach ($satuanBahan as $select)
                                        <option value="{{ $select->id }}"
                                            {{ $item->satuanBahanId == $select->id ? 'selected' : '' }}>
                                            {{ $select->satuanBahan }}
                                        </option>
                                    @endforeach
                                </select>
